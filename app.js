...
function renderMonthsGrid() {
  ...
  MONTHS.forEach((month, idx) => {
    ...
    monthBtn.className = `month-card px-4 py-3 rounded-xl border font-semibold text-xs uppercase transition-all ${isAvailable ? "hover:bg-indigo-500/10 hover:border-indigo-500/30 cursor-pointer" : "cursor-not-allowed opacity-40"} ${isCached ? "cached" : ""}`;
    monthBtn.style.cssText = isAvailable ? `border-color: var(--border-color); background: var(--input-bg); color: var(--text-secondary);` : `border-color: var(--border-color); background: rgba(0,0,0,0.1); color: var(--text-tertiary);`;
...
async function globalSearch() {
  ...
  for (let fileName of fileNames) {
    ...
    setStatus(cached ? `Searching ${fileName.toUpperCase()} (cached)...` : `Fetching ${fileName.toUpperCase()}...`, true, true);

    try {
      const data = await fetchAndCache(fileName);
      if (!data) continue;

      const matches = data.filter((row) => row[0].includes(query) || row[6].includes(query) || row[7].includes(query) || row[8].includes(query));
